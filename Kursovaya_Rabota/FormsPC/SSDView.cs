    ...
    public partial class SSDView : Form
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                ConnectStaff.Open();
                string sqlSocket = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 56 AND Properties_value.Item_ID = " + id;
                MySqlCommand cmd1 = new MySqlCommand(sqlSocket, ConnectStaff);
                socket = cmd1.ExecuteScalar().ToString();
                label12.Text = socket;

                string sqlchipset = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 55 AND Properties_value.Item_ID = " + id;
                ...
                string sqltypemem = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 54 AND Properties_value.Item_ID = " + id;
                ...
                string sqlssdm = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 53 AND Properties_value.Item_ID = " + id;
                MySqlCommand cmdThread = new MySqlCommand(sqlssdm, ConnectStaff);
                ssdm = cmdThread.ExecuteScalar().ToString();
                label15.Text = ssdm;

                string sqlsata = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 51 AND Properties_value.Item_ID = " + id;
                MySqlCommand cmdTDP = new MySqlCommand(sqlsata, ConnectStaff);
                sata = cmdTDP.ExecuteScalar().ToString();
                label17.Text = sata;

                string sqlvideo = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 50 AND Properties_value.Item_ID = " + id;
                MySqlCommand cmdvideo = new MySqlCommand(sqlvideo, ConnectStaff);
                video = cmdvideo.ExecuteScalar().ToString();
                label16.Text = video;

                string sqlpowercpu = "SELECT Properties_value.`Value` FROM Properties_value WHERE Properties_value.Property_ID = 52 AND Properties_value.Item_ID = " + id;
